...
var DataHandler = /** @class */ (function () {
    ...
    DataHandler.prototype.loadData = function (path) {
        if (path === void 0) { path = null; }
        if (path) {
            var model_id = this.watchPaths.indexOf(path).toString();
            this.models[model_id] = JSON.parse(fs_1.default.readFileSync(path, { encoding: "utf-8" }));
        }
        else {
    ...
    DataHandler.prototype.startWatcher = function () {
        ...
        this.watchPaths.forEach(function (path) {
            ...
            fs_1.default.watchFile(path, { interval: _this.update_interval }, function () {
                console.log("Model " + _this.watchPaths.indexOf(path).toString() + " updated.");
                _this.loadData(path);
